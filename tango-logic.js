...
class Session{
  constructor(board,options={}){
    ...
    this.relationFacts=new Map();this.relationClosure=new Map();this.relationConflict=null;this.appliedDeductions=[];
  ...
  clone(){let x=Object.create(Session.prototype);x.n=this.n;x.quota=this.quota;x.state=cloneGrid(this.state);x.edges=copy(this.edges);x.options={...this.options};x.factSeq=this.factSeq;x.dedSeq=this.dedSeq;x.valueFacts=this.valueFacts.map(row=>row.map(f=>f?copy(f):null));x.relationFacts=new Map([...this.relationFacts].map(([k,v])=>[k,copy(v)]));x.relationClosure=new Map([...this.relationClosure].map(([k,v])=>[k,copy(v)]));x.relationConflict=this.relationConflict?copy(this.relationConflict):null;x.appliedDeductions=this.appliedDeductions.map(copy);return x}
  ...
  addValue(cell,value,meta={}){let [r,c]=cell,old=this.valueFacts[r][c];if(old)return old.value===value?old:null;if(this.state[r][c]!==VALUE_EMPTY&&this.state[r][c]!==value)return null;this.state[r][c]=value;let f={id:'F'+(++this.factSeq),kind:'VALUE',cell:cell.slice(),value,rank:Number(meta.rank)||0,source:meta.source||'deduction',deductionId:meta.deductionId||null,hypothesis:!!meta.hypothesis};this.valueFacts[r][c]=f;return f}
  ...
  rebuildRelationClosure(){
    this.relationClosure=new Map();if(this.relationConflict)return;
  ...
  applyDeduction(d,options={}){
    ...
    let applied=copy(d);applied.id='D'+(++this.dedSeq);let changed=false,beforeRelations=new Set(this.relationClosure.keys());
    for(const c of applied.conclusions){
      ...
      else if(c.type==='RELATION'){let old=this.relationBetween(c.a,c.b);if(old&&old.parity===c.parity&&old.rank<=applied.rank)continue;let f=this.addBaseRelation(c.a,c.b,c.parity,{rank:applied.rank,deductionId:applied.id,dependencies:[applied.id],source:'derived'});if(f)changed=true}
    }
    if(!changed)return {deduction:null,automatic:[]};this.appliedDeductions.push(applied);this.rebuildRelationClosure();let automatic=this.closureRelationDeductions(beforeRelations).map(d=>{let a=copy(d);a.id='D'+(++this.dedSeq);this.appliedDeductions.push(a);return a});
  ...
  hypothesisResult(cell,value,maxSteps=this.options.maxHypothesisSteps){
    let fork=this.clone();if(!fork.assume(cell,value))return {contradiction:{kind:'VALUE_CONFLICT',cells:[cell.slice()]},trace:[],session:fork};let trace=[],bad=fork.closeCost0(trace);if(bad)return {contradiction:bad,trace,session:fork};
    for(let i=0;i<maxSteps;i++){bad=fork.diagnose();if(bad)return {contradiction:bad,trace,session:fork};let d=fork.bestDirect();if(!d)break;let a=fork.applyDeduction(d);if(!a.deduction)break;trace.push(a.deduction,...a.automatic);bad=fork.closeCost0(trace);if(bad)return {contradiction:bad,trace,session:fork}}
    return {contradiction:fork.diagnose(),trace,session:fork}
  }
  ...
  contradictionDeduction(cell,rejected,result){let chosen=1-rejected,ps=this.realTracePremises([result]);ps.unshift({kind:'ASSUMPTION',cell:cell.slice(),value:rejected,rank:0,dependencies:[],hypothesis:true});let witness=copy(result.contradiction),focus=uniqCells([cell].concat(witness?.cells||[]));return this.makeDeduction('ASSUMPTION_CONTRADICTION',3,ps,focus,[],witness?.family!=null?[unitRef(witness.family,witness.id)]:[],[{type:'VALUE',cell,value:chosen}],{assumption:{cell:cell.slice(),value:rejected},contradictionType:witness?.kind||'UNKNOWN',witness,trace:(result.trace||[]).map(d=>({rule:d.rule,rank:d.rank,techniqueLevel:d.techniqueLevel,conclusions:copy(d.conclusions||[])}))},90,45)}
  findAssumptionContradictions(limit=4){let out=[];for(let r=0;r<this.n;r++)for(let c=0;c<this.n;c++)if(this.valueAt([r,c])===VALUE_EMPTY){let a=[this.hypothesisResult([r,c],VALUE_MOON),this.hypothesisResult([r,c],VALUE_SUN)];if(!!a[0].contradiction===!!a[1].contradiction)continue;let rejected=a[0].contradiction?VALUE_MOON:VALUE_SUN;out.push(this.contradictionDeduction([r,c],rejected,a[rejected]));if(out.length>=limit)return out.sort(deductionComparator)}return out.sort(deductionComparator)}
  ...
  findCommonConsequences(limit=2){let out=[];for(let r=0;r<this.n;r++)for(let c=0;c<this.n;c++)if(this.valueAt([r,c])===VALUE_EMPTY){let a=this.hypothesisResult([r,c],VALUE_MOON,this.options.maxCommonSteps),b=this.hypothesisResult([r,c],VALUE_SUN,this.options.maxCommonSteps);if(a.contradiction||b.contradiction)continue;let fa=this.commonFacts(a),fb=this.commonFacts(b),conclusion=null;for(const [k,x] of fa.values){let y=fb.values.get(k);if(y&&y.value===x.value&&!sameCell(x.cell,[r,c])){conclusion=x;break}}if(!conclusion)for(const [k,x] of fa.relations){let y=fb.relations.get(k);if(y&&y.parity===x.parity&&!sameCell(x.a,[r,c])&&!sameCell(x.b,[r,c])){conclusion=x;break}}if(!conclusion)continue;let ps=this.realTracePremises([a,b]);ps.unshift({kind:'ASSUMPTION_SPLIT',cell:[r,c],rank:0,dependencies:[]});out.push(this.makeDeduction('COMMON_CONSEQUENCE',3,ps,uniqCells([[r,c]].concat(conclusion.type==='VALUE'?[conclusion.cell]:[conclusion.a,conclusion.b])),conclusion.type==='RELATION'?[{a:conclusion.a,b:conclusion.b,parity:conclusion.parity,relation:relationName(conclusion.parity)}]:[],[],[conclusion],{branchCell:[r,c],moonTrace:a.trace.map(d=>({rule:d.rule,conclusions:copy(d.conclusions||[])})),sunTrace:b.trace.map(d=>({rule:d.rule,conclusions:copy(d.conclusions||[])}))},100,55));if(out.length>=limit)return out.sort(deductionComparator)}return out.sort(deductionComparator)}
  nextDeduction(){let contradiction=this.diagnose();if(contradiction)return {contradiction};let direct=this.bestDirect();if(direct)return {deduction:direct};let hypo=this.findAssumptionContradictions();if(hypo.length)return {deduction:hypo[0]};let common=this.findCommonConsequences();return {deduction:common[0]||null}}
